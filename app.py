...
SESSION_TTL_SECONDS = 60 * 60 * 8  # 8 hours demo login session for Streamlit link navigation
...
# =============================
# LINK-SAFE SESSION HELPERS
# =============================
@st.cache_resource
def get_browser_session_store():
    """
    Small server-side token store for this Streamlit app.

    Why this is needed:
    - Movie cards are normal HTML <a> links.
    - Link navigation can refresh the Streamlit app URL.
    - If auth exists only in st.session_state, the user may be sent back to login.
    - This token lets the app restore the logged-in user after card/nav links.
    """
    return {}
...
def create_browser_session(user: dict) -> str:
    ...
    get_browser_session_store()[token] = {
        "user": user,
...
def clear_browser_session(token: str | None = None):
    token = token or st.session_state.get("session_token") or st.query_params.get("session")
    if token:
        get_browser_session_store().pop(token, None)


def restore_browser_session_from_url():
    token = st.query_params.get("session")
    if not token:
        return

    record = get_browser_session_store().get(token)
    if not record:
        return
    ...
    st.session_state.user = record.get("user")


def current_session_token():
    return st.session_state.get("session_token") or st.query_params.get("session")


def app_url(**params) -> str:
    """
    Build internal links while preserving the login session token.
    Example: ?view=details&id=123&session=abc
    """
    query = {}
    ...
    if token and "auth" not in query:
        query["session"] = token

    return "?" + urlencode(query)


def set_app_query(**params):
    """Update URL query params without dropping the active login session token."""
    token = current_session_token()
...
restore_browser_session_from_url()
...
def login_user(user: dict):
    token = create_browser_session(user)
    st.session_state.authenticated = True
...
def render_header():
    ...
    header_html = (
        ...
        "</div>"
        "<div class='nav-menu'>"
        f"<a class='nav-link {nav_active(view='home')}' href='{app_url(view='home')}'>Home</a>"
        f"<a class='nav-link {nav_active(cat='trending')}' href='{app_url(view='home', cat='trending')}'>Trending</a>"
        f"<a class='nav-link {nav_active(cat='popular')}' href='{app_url(view='home', cat='popular')}'>Popular</a>"
        f"<a class='nav-link {nav_active(cat='top_rated')}' href='{app_url(view='home', cat='top_rated')}'>Top Rated</a>"
        f"<a class='nav-link {nav_active(cat='now_playing')}' href='{app_url(view='home', cat='now_playing')}'>Now Playing</a>"
        f"<a class='nav-link {nav_active(view='account')} nav-user' href='{app_url(view='account')}'>👤 {username}</a>"
        f"<a class='nav-link {nav_active(view='help')}' href='{app_url(view='help')}'>Help</a>"
        f"<a class='nav-link' href='{app_url(auth='logout')}'>Logout</a>"
        "</div>"
        "</div>"
    )
...
def poster_grid(cards, cols=6):
    ...
    for movie in cards:
        ...
        href = app_url(view="details", id=tmdb_id)
    ...
    if typed.strip():
        ...
        if suggestions:
            ...
            if selected != "-- Select a movie --":
                ...
                st.session_state.view = "details"
                set_app_query(view="details", id=int(selected_id))
                st.rerun()
        else:
